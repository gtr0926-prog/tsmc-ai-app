...
st.markdown("""
<div style="background-color:#fff3e0;padding:15px;border-radius:10px;margin-bottom:20px;border-left: 5px solid #ff9800;">
...
# --- 2. 側邊欄參數 ---
# 修改：使用 markdown 與 HTML 強制設定標題顏色為白色
...
adjust_points = series_pred[signals == -1].index # 原本的 Exit 改為 Adjust
...
fig.add_trace(go.Scatter(
    ...
    mode='lines', 
    name='AI 合理趨勢線', 
    line=dict(color='#ff6d00', width=2) # 移除 dash='dash' 改為實線
), row=1, col=1)
...
# 4. 調節訊號 (Adjust) (上圖) - 改為橘色圓形或向下箭頭
fig.add_trace(go.Scatter(
    x=adjust_points, 
    y=adjust_prices * 1.02, 
    mode='markers', 
    name='AI 調節訊號 (Adjust)', 
    marker=dict(color='#ff9100', size=12, symbol='circle-dot'), # 橘色代表調節
    text='調節 (減碼)',
    hoverinfo='x+y+name'
), row=1, col=1)
...
fig.add_trace(go.Scatter(
    ...
    mode='lines+markers',
    name='未來 3 日預測',
    line=dict(color='#6200ea', width=3, dash='dot'),
    marker=dict(size=8)
), row=1, col=1)

# 6. 成交量柱狀圖 (下圖) - 模擬外資/主力動能
# 邏輯：紅K量 (收>開) 為紅色，綠K量 (收<開) 為綠色
